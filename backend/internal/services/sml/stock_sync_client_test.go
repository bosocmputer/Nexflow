...
func TestStockSyncClientSendsTenantAndTypedBatch(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(req.Scopes) != 1 || req.Scopes[0].Locations[0].Warehouse != "01" {
	...
	result, err := client.BalancesBatch(context.Background(), StockBalanceBatchRequest{
		AsOfDate: "2026-08-10",
		Scopes:   []StockBalanceScopeRequest{{ScopeID: "shop:1", ItemCodes: []string{"SKU1"}, ScopeMode: "selected", Locations: []StockLocationPair{{Warehouse: "01", Location: "01"}}}},
	})
	if err != nil {
		t.Fatal(err)
	}
