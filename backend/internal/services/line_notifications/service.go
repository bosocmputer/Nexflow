...
// EnqueueTikTokSettlementResult creates a durable, PII-free terminal
// settlement notification. A Flex payload is preferred so it follows the
// existing Shopee/TikTok Marketplace hierarchy; plain text remains a delivery
// fallback when a rich payload cannot be generated or is disabled.
func (s *Service) EnqueueTikTokSettlementResult(ctx context.Context, in models.TikTokSettlementLineNotification, dedupeKey string) (int, error) {
	if s == nil || s.repo == nil || !s.settlementLineAlertsEnabled || strings.TrimSpace(in.RunID) == "" {
		return 0, nil
	}
	dedupeKey = strings.TrimSpace(dedupeKey)
	if dedupeKey == "" {
		dedupeKey = "tiktok:settlement:" + strings.TrimSpace(in.RunID) + ":" + strings.TrimSpace(in.Outcome)
	}
	title, severity := tikTokSettlementNotificationMeta(in.Outcome)
	message := BuildTikTokSettlementLineText(in, s.publicBaseURL)
	altText := ""
	var flexPayload json.RawMessage
	payloadVersion := 0
	if s.richFlexEnabled {
		if alt, contents := BuildTikTokSettlementLineFlex(in, s.publicBaseURL); contents != nil {
			if raw, err := json.Marshal(contents); err == nil {
				altText, flexPayload, payloadVersion = alt, raw, 1
			} else if s.logger != nil {
				s.logger.Warn("line TikTok settlement flex marshal failed", zap.String("run_id", in.RunID), zap.Error(err))
			}
		}
	}
	return s.repo.Enqueue(ctx, models.LineNotificationMessageInput{
		Source: "tiktok_settlement", Severity: severity, Title: title,
		Body: tikTokSettlementNotificationBody(in), ActionURL: TikTokSettlementActionURL(s.publicBaseURL),
		EntityType: "tiktok_settlement", EntityID: strings.TrimSpace(in.RunID), DedupeKey: dedupeKey,
		MessageText: message, AltText: altText, FlexPayload: flexPayload, PayloadVersion: payloadVersion,
	})
}
...
func tikTokSettlementNotificationMeta(outcome string) (title, severity string) {
	switch strings.TrimSpace(outcome) {
	case "sent":
		return "ส่ง RC TikTok Shop แล้ว", "info"
	case "unknown_result":
		return "ต้องตรวจผล RC TikTok Shop", "warning"
	default:
		return "ส่ง RC TikTok Shop ไม่สำเร็จ", "error"
	}
}

func tikTokSettlementNotificationBody(in models.TikTokSettlementLineNotification) string {
	parts := []string{strings.TrimSpace(in.ShopName)}
	if in.OrderCount > 0 {
		parts = append(parts, fmt.Sprintf("%d คำสั่งซื้อ", in.OrderCount))
	}
	if in.TotalAmount != 0 {
		parts = append(parts, "ยอด "+tikTokSettlementLineMoney(in.TotalAmount, in.Currency))
	}
	if docNo := strings.TrimSpace(in.RCDocNo); docNo != "" {
		parts = append(parts, "เอกสาร "+docNo)
	}
	if message := compactWhitespace(in.ErrorMessage); message != "" && in.Outcome != "sent" {
		parts = append(parts, truncateRunes(message, 180))
	}
	return strings.Join(filterNonEmpty(parts), " · ")
}

func tikTokSettlementLineMoney(amount float64, currency string) string {
	currency = strings.ToUpper(strings.TrimSpace(currency))
	if currency == "" {
		currency = "THB"
	}
	return formatLineMoneyValue(amount) + " " + currency
}

func BuildTikTokSettlementLineText(in models.TikTokSettlementLineNotification, publicBaseURL string) string {
	title, _ := tikTokSettlementNotificationMeta(in.Outcome)
	shop := fallbackDash(strings.TrimSpace(in.ShopName))
	parts := []string{title, "ร้าน: " + shop}
	if in.TotalAmount != 0 {
		parts = append(parts, "ยอด TikTok: "+tikTokSettlementLineMoney(in.TotalAmount, in.Currency))
	}
	if in.OrderCount > 0 {
		parts = append(parts, fmt.Sprintf("จำนวนคำสั่งซื้อ: %d รายการ", in.OrderCount))
	}
	parts = append(parts,
		"Statement: "+fallbackDash(strings.TrimSpace(in.StatementID)),
		"Payment ID: "+fallbackDash(strings.TrimSpace(in.PaymentID)),
	)
	if docNo := strings.TrimSpace(in.RCDocNo); docNo != "" {
		parts = append(parts, "เลขเอกสาร SML: "+docNo)
	}
	...
	if actionURL := TikTokSettlementActionURL(publicBaseURL); actionURL != "" {
		parts = append(parts, "เปิดใน Nexflow: "+actionURL)
	}
	return strings.Join(filterNonEmpty(parts), "\n")
}

// BuildTikTokSettlementLineFlex uses the same source chip and spacing as
// TikTok order/SML messages. The black chip distinguishes the channel without
// adding a full-width header bar.
func BuildTikTokSettlementLineFlex(in models.TikTokSettlementLineNotification, _ string) (string, map[string]any) {
	...
	accent := "#16A34A"
	if in.Outcome == "unknown_result" {
		accent = "#D97706"
	} else if in.Outcome != "sent" {
		accent = "#DC2626"
	}
	alt := strings.Join(filterNonEmpty([]string{title, shop, strings.TrimSpace(in.RCDocNo)}), " · ")
	body := []map[string]any{
		flexMarketplaceSourceChip("TikTok Shop", tikTokFlexBaseColor),
		flexText(title, "lg", "bold", "#0F172A", "", true),
		flexText(shop, "sm", "", "#64748B", "", true),
	...
	if in.TotalAmount != 0 {
		body = append(body, flexAmountRow("ยอด TikTok", tikTokSettlementLineMoney(in.TotalAmount, in.Currency), accent))
	}
	orderCount := ""
	if in.OrderCount > 0 {
		orderCount = fmt.Sprintf("%d รายการ", in.OrderCount)
	}
	body = appendFlexSection(body, "รอบรับชำระ", []flexKVRow{
		{Label: "Statement", Value: strings.TrimSpace(in.StatementID)},
		{Label: "Payment ID", Value: strings.TrimSpace(in.PaymentID)},
		{Label: "จำนวนคำสั่งซื้อ", Value: orderCount},
		{Label: "เลขเอกสาร SML", Value: strings.TrimSpace(in.RCDocNo)},
	})
	if message := compactWhitespace(in.ErrorMessage); message != "" && in.Outcome != "sent" {
		body = append(body,
			map[string]any{"type": "separator", "margin": "md"},
...
func TikTokSettlementActionURL(publicBaseURL string) string {
	path := "/tiktok-settlements"
	base := strings.TrimRight(strings.TrimSpace(publicBaseURL), "/")
	if base == "" {
		return path
	}
