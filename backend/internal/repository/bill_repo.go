...
// DashboardMonitorSummary is a read-only executive view of the Marketplace
// sales flow. Amounts use the date the event occurred: Marketplace order
// creation, Nexflow Bill creation, and a successful SML send respectively.
// They are intentionally not subtracted from one another because an order may
// cross a date boundary between steps.
type DashboardMonitorSummary struct {
	Flow      []DashboardFlowPlatform `json:"flow"`
	Backlog   DashboardFlowBacklog    `json:"backlog"`
	Stock     []DashboardStockUnit    `json:"stock"`
	CheckedAt time.Time               `json:"checked_at"`
}

type DashboardFlowPlatform struct {
	Platform    string  `json:"platform"`
	Incoming    int     `json:"incoming_count"`
	IncomingAmt float64 `json:"incoming_amount"`
	Bills       int     `json:"bills_created_count"`
	BillsAmt    float64 `json:"bills_created_amount"`
	SMLSent     int     `json:"sml_sent_count"`
	SMLSentAmt  float64 `json:"sml_sent_amount"`
}

type DashboardFlowBacklog struct {
	AwaitingDocument int `json:"awaiting_document"`
	AwaitingSML      int `json:"awaiting_sml"`
	FailedSML        int `json:"failed_sml"`
}

// DashboardStockUnit is the latest stored dry-run snapshot. It does not
// query SML when an executive opens Dashboard, so it always exposes the last
// checked time rather than claiming a live stock balance.
type DashboardStockUnit struct {
	UnitCode        string     `json:"unit_code"`
	PoolCount       int        `json:"pool_count"`
	SMLAvailableQty float64    `json:"sml_available_qty"`
	ShopeeTargetQty int64      `json:"shopee_target_qty"`
	TikTokTargetQty int64      `json:"tiktok_target_qty"`
...
// DashboardMonitorSummaryForDateRange loads only persisted tenant data. It is
// safe for Dashboard because it neither calls a Marketplace/SML API nor starts
// a worker. The optional TikTok and stock tables are skipped on older tenants.
func (r *BillRepo) DashboardMonitorSummaryForDateRange(ctx context.Context, fromDate, toDate string) (DashboardMonitorSummary, error) {
	window, err := buildPlatformSalesWindowForDateRange(time.Now(), fromDate, toDate)
	if err != nil {
		return DashboardMonitorSummary{}, err
	}
	result := DashboardMonitorSummary{Flow: make([]DashboardFlowPlatform, 0, len(platformSalesOrder)), Stock: []DashboardStockUnit{}, CheckedAt: time.Now().UTC()}
	flow := map[string]*DashboardFlowPlatform{}
	for _, platform := range platformSalesOrder {
		flow[platform] = &DashboardFlowPlatform{Platform: platform}
	}

	// Excel imports have no separately persisted Marketplace-order queue, so
	// their imported Bill is the first durable event. Shopee and TikTok use
	// their typed snapshot tables as the incoming-order source.
	if err := r.db.QueryRowContext(ctx, `SELECT
		COUNT(*) FILTER (WHERE status IN ('needs_review','pending'))::int,
		COUNT(*) FILTER (WHERE status='failed')::int
		FROM bills
		WHERE bill_type='sale' AND archived_at IS NULL AND source IN ('shopee','lazada','tiktok')`).Scan(
		&result.Backlog.AwaitingSML, &result.Backlog.FailedSML,
	); err != nil {
		return DashboardMonitorSummary{}, fmt.Errorf("load dashboard SML backlog: %w", err)
	}

	rows, err := r.db.QueryContext(ctx, `SELECT source,
		COUNT(*) FILTER (WHERE created_at >= $1 AND created_at < $2)::int,
		COALESCE(SUM(GREATEST(COALESCE(amount,0),0)) FILTER (WHERE created_at >= $1 AND created_at < $2),0)::float8,
		COUNT(*) FILTER (WHERE status='sent' AND sent_at >= $1 AND sent_at < $2)::int,
		COALESCE(SUM(GREATEST(COALESCE(amount,0),0)) FILTER (WHERE status='sent' AND sent_at >= $1 AND sent_at < $2),0)::float8
		FROM (
			SELECT b.source,b.status,b.id,b.created_at,b.sent_at,
			...
			WHERE b.bill_type='sale' AND b.archived_at IS NULL AND b.source IN ('shopee','lazada','tiktok')
				AND ((b.created_at >= $1 AND b.created_at < $2) OR (b.sent_at >= $1 AND b.sent_at < $2))
			GROUP BY b.source,b.status,b.id,b.created_at,b.sent_at
		) bill_events GROUP BY source`, window.fromTime, window.toExclusive)
	if err != nil {
		return DashboardMonitorSummary{}, fmt.Errorf("load dashboard Bill flow: %w", err)
	}
	for rows.Next() {
		var platform string
		var bills, sent int
		var billsAmt, sentAmt float64
		if err := rows.Scan(&platform, &bills, &billsAmt, &sent, &sentAmt); err != nil {
			rows.Close()
			return DashboardMonitorSummary{}, err
		}
		if item := flow[platform]; item != nil {
			item.Bills, item.BillsAmt, item.SMLSent, item.SMLSentAmt = bills, billsAmt, sent, sentAmt
		}
	}
	...
	if err := r.dashboardMonitorShopeeIncoming(ctx, window, flow["shopee"]); err != nil {
		return DashboardMonitorSummary{}, err
	}
	if exists, err := r.dashboardRelationExists(ctx, "tiktok_shop_order_snapshots"); err != nil {
		return DashboardMonitorSummary{}, err
	} else if exists {
		if err := r.dashboardMonitorTikTokIncoming(ctx, window, flow["tiktok"]); err != nil {
			return DashboardMonitorSummary{}, err
		}
	}
	// Lazada Excel has no live order snapshot; its import creates the local Bill.
	flow["lazada"].Incoming, flow["lazada"].IncomingAmt = flow["lazada"].Bills, flow["lazada"].BillsAmt

	work, err := r.DashboardWorkSummary(ctx)
	if err != nil {
		return DashboardMonitorSummary{}, err
	}
	result.Backlog.AwaitingDocument = work.Documents.Shopee + work.Documents.TikTok
	for _, platform := range platformSalesOrder {
		result.Flow = append(result.Flow, *flow[platform])
	}

	if exists, err := r.dashboardRelationExists(ctx, "marketplace_stock_pools"); err != nil {
		return DashboardMonitorSummary{}, err
	} else if exists {
		stock, err := r.dashboardStockUnits(ctx)
		if err != nil {
			return DashboardMonitorSummary{}, err
		}
		result.Stock = stock
...
func (r *BillRepo) dashboardMonitorShopeeIncoming(ctx context.Context, window platformSalesWindow, result *DashboardFlowPlatform) error {
	return r.db.QueryRowContext(ctx, `SELECT COUNT(*)::int,COALESCE(SUM(GREATEST(COALESCE(total_amount,0),0)),0)::float8
		FROM shopee_order_snapshots
		WHERE order_status NOT IN ('CANCELLED','IN_CANCEL','UNPAID')
		AND (CASE WHEN raw_detail->>'create_time' ~ '^[0-9]+$' THEN to_timestamp((raw_detail->>'create_time')::bigint) ELSE created_at END) >= $1
		AND (CASE WHEN raw_detail->>'create_time' ~ '^[0-9]+$' THEN to_timestamp((raw_detail->>'create_time')::bigint) ELSE created_at END) < $2`, window.fromTime, window.toExclusive).Scan(&result.Incoming, &result.IncomingAmt)
}

func (r *BillRepo) dashboardMonitorTikTokIncoming(ctx context.Context, window platformSalesWindow, result *DashboardFlowPlatform) error {
	return r.db.QueryRowContext(ctx, `SELECT COUNT(*)::int,COALESCE(SUM(GREATEST(COALESCE(payment_total_amount,0),0)),0)::float8
		FROM tiktok_shop_order_snapshots
		WHERE order_status NOT IN ('CANCELLED','UNPAID')
		AND (CASE WHEN safe_order->>'create_time' ~ '^[0-9]+$' THEN to_timestamp((safe_order->>'create_time')::bigint) ELSE created_at END) >= $1
		AND (CASE WHEN safe_order->>'create_time' ~ '^[0-9]+$' THEN to_timestamp((safe_order->>'create_time')::bigint) ELSE created_at END) < $2`, window.fromTime, window.toExclusive).Scan(&result.Incoming, &result.IncomingAmt)
}

func (r *BillRepo) dashboardStockUnits(ctx context.Context) ([]DashboardStockUnit, error) {
	rows, err := r.db.QueryContext(ctx, `WITH latest_preview AS (
		SELECT DISTINCT ON (pool_id) pool_id,summary,finished_at
		FROM marketplace_stock_runs
		WHERE run_type='preview' AND status='success'
		ORDER BY pool_id,finished_at DESC NULLS LAST,id DESC
	), members AS (
		SELECT pool_id,
			COALESCE(SUM(last_target_qty) FILTER (WHERE source='shopee' AND enabled),0)::bigint AS shopee_target_qty,
			COALESCE(SUM(last_target_qty) FILTER (WHERE source='tiktok' AND enabled),0)::bigint AS tiktok_target_qty
		FROM marketplace_stock_pool_members GROUP BY pool_id
	)
		SELECT p.sml_unit_code,COUNT(*)::int,
			COALESCE(SUM(COALESCE((lp.summary->>'sml_available_qty')::numeric,0)),0)::float8,
			COALESCE(SUM(m.shopee_target_qty),0)::bigint,COALESCE(SUM(m.tiktok_target_qty),0)::bigint,
			MAX(lp.finished_at)
		FROM marketplace_stock_pools p
		LEFT JOIN latest_preview lp ON lp.pool_id=p.id
		LEFT JOIN members m ON m.pool_id=p.id
		WHERE p.archived_at IS NULL
		GROUP BY p.sml_unit_code ORDER BY p.sml_unit_code`)
	if err != nil {
		return nil, fmt.Errorf("load dashboard stock snapshot: %w", err)
	}
	defer rows.Close()
	result := []DashboardStockUnit{}
	for rows.Next() {
		var item DashboardStockUnit
		if err := rows.Scan(&item.UnitCode, &item.PoolCount, &item.SMLAvailableQty, &item.ShopeeTargetQty, &item.TikTokTargetQty, &item.LastPreviewAt); err != nil {
			return nil, err
		}
