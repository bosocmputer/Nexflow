...
func (c *TokenCipher) Encrypt(plaintext string, aad []byte) (ciphertext, nonce []byte, err error) {
	...
	return c.aead.Seal(nil, nonce, []byte(plaintext), aad), nonce, nil
}
...
func tokenAAD(tenant, shopID, tokenKind string) []byte {
	return []byte(strings.ToLower(strings.TrimSpace(tenant)) + ":" + strings.TrimSpace(shopID) + ":" + strings.TrimSpace(tokenKind))
}
...
func DeriveTenantSecret(encodedMasterKey, tenant string) (string, error) {
	...
	if !tenantSlugPattern.MatchString(tenant) {
		return "", errors.New("tenant is invalid")
	}
	mac := hmac.New(sha256.New, master)
	_, _ = mac.Write([]byte("nexflow-tiktok-shop-gateway/tenant/" + tenant))
