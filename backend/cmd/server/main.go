...
func main() {
	...
	marketplacestock.NewSyncWorker(marketplaceStockStore, marketplaceStockService, stockShopeeClient, tiktokGatewayClient, cfg.MarketplaceStockWriteEnabled, logger).Start(appCtx)
	...
	api := r.Group("/api", middleware.Auth())
	{
		...
		api.GET("/settings/shopee-stock", middleware.RequireRole("admin", "staff"), shopeeStockH.Overview)
		api.GET("/settings/shopee-stock/catalog-search", middleware.RequireRole("admin", "staff"), shopeeStockH.SearchCatalog)
		api.GET("/settings/shopee-stock/:shop_id/product-groups", middleware.RequireRole("admin", "staff"), shopeeStockH.ProductGroups)
		api.GET("/settings/shopee-stock/:shop_id/product-groups/:item_id/variants", middleware.RequireRole("admin", "staff"), shopeeStockH.ProductGroupVariants)
		...
		api.GET("/settings/shopee-stock/:shop_id/runs/:run_id", middleware.RequireRole("admin", "staff"), shopeeStockH.PreviewRun)
		api.GET("/settings/shopee-stock/:shop_id/runs/:run_id/lines", middleware.RequireRole("admin", "staff"), shopeeStockH.PreviewRunLines)
		...
		api.GET("/tiktok-shop-api/local-connections", middleware.RequireRole("admin", "staff"), tiktokAPIH.ListLocalConnections)
		...
		api.GET("/tiktok-settlements/settings", middleware.RequireRole("admin", "staff"), tiktokSettlementH.Settings)
		api.GET("/tiktok-settlements/route", middleware.RequireRole("admin", "staff"), tiktokSettlementH.RouteSummary)
		...
		api.POST("/tiktok-settlements/income-exports/preview", middleware.RequireRole("admin", "staff"), tiktokSettlementH.PreviewIncomeExport)
		api.POST("/tiktok-settlements/income-exports/receipt-candidates", middleware.RequireRole("admin", "staff"), tiktokSettlementH.CreateIncomeReceiptCandidate)
