...
| `/settings/catalog` | `CatalogSettings` | `GET /api/catalog`, `GET /api/catalog/stats`, `GET /api/catalog/hidden-codes`, `POST /api/catalog/sync`, `POST /api/catalog/:code/refresh`, `POST /api/catalog/refresh-batch`, `DELETE /api/catalog/:code` | full sync, refresh/delete catalog rows | loading, sync running, empty catalog, hidden-code warnings | read authenticated; unit/product create admin/staff; sync/refresh/delete admin |
| `/settings/shopee-stock` | `ShopeeStock` | overview, grouped product/variant pages, preview-run progress/lines, catalog search, shared-pool detail | settings/catalog sync, async dry-run, live stock sync, Product Master mapping, shared-pool allocation | unavailable shop, read-only staff, stale/dry-run-required, grouped loading/error/empty states, blocked/unknown run details | admin/staff read; every `POST`/`PUT` mutation admin only |
...
| `/settings/instance` | `InstanceSettings` | `GET/PUT /api/settings/instance`, `GET /api/setup/status?summary=1`, `GET /api/settings/shopee-api/status?summary=1`, `GET /api/settings/line-notifications/status` | edit store name/support contact only; integration status is read-only | independent loading/error states, checked time, manual refresh | admin only |
| `/settings/line-notifications` | `LineNotifications` | `GET /api/settings/line-notifications`, `GET /api/settings/line-notifications/status`, sender/recipient CRUD, sender test, recipient rich Flex test | create/update/delete LINE sender/recipient; test push consumes LINE push quota | readiness cards, masked IDs, recent delivery states, rich Flex fallback sample, failed test error | admin only |
