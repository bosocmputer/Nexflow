...
export default function TikTokSettlement() {
  ...
  useEffect(() => {
    void load()
  }, [shopID, from, to, runStatus, paymentStatus])
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-2 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <p className="sr-only">ดูหลักฐานรอบถอนเงิน TikTok Shop โดยระบบจะไม่สร้างเอกสารรับชำระหนี้จากยอดที่คาดเดา</p>
        ...
        <div className="mt-2 space-y-2 border-t border-border/60 pt-2">
          <div className="grid gap-1.5 sm:grid-cols-2 lg:flex lg:flex-wrap lg:items-center">
            ...
            <Select value={paymentStatus} onValueChange={setPaymentStatus}>
              <SelectTrigger className="h-8 w-full text-xs sm:w-[170px]" aria-label="กรองตามสถานะ Statement">
                <SelectValue placeholder="สถานะ Statement" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">ทุกสถานะ Statement</SelectItem>
                <SelectItem value="PAID">TikTok โอนแล้ว</SelectItem>
                <SelectItem value="PROCESSING">รอ TikTok โอน</SelectItem>
                <SelectItem value="FAILED">TikTok โอนไม่สำเร็จ</SelectItem>
              </SelectContent>
            </Select>
      ...
      <Card className="border-border/70 shadow-none">
        <CardContent className="p-0">
          {loading && <div className="p-6 text-sm text-muted-foreground">กำลังโหลด Statement…</div>}
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full text-sm">
              <thead className="border-b bg-muted/30 text-left text-muted-foreground">
                <tr>
                  <th className="p-3">Statement / วันที่</th>
              ...
              <tbody>
                {runs.map((run) => (
                  <tr key={run.id} className="border-b last:border-0">
                    <td className="p-3">
                      <p className="font-medium">{run.statement_id}</p>
                    ...
                    <td className="p-3 text-right">
                      ...
                      <p className="text-xs text-muted-foreground">{run.item_count ?? '-'} order</p>
                    </td>
                    <td className="p-3">
          ...
          {!loading && runs.length === 0 && (
            <div className="p-10 text-center text-sm text-muted-foreground">
              ยังไม่มี Statement ในช่วงนี้ การเปิดหน้านี้จะไม่เรียก TikTok Finance อัตโนมัติ
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(openDialog) => !openDialog && setSelected(null)}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><WalletCards className="h-5 w-5" />Statement {selected?.statement_id}</DialogTitle>
            <DialogDescription>TikTok แจ้งว่าโอนแล้ว เป็นหลักฐานจากแพลตฟอร์ม ไม่ใช่การยืนยันจาก statement ธนาคาร</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={withdrawalOpen} onOpenChange={setWithdrawalOpen}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              แสดงเฉพาะรายการที่กดถอนเงินในช่วงวันที่เลือก TikTok Shop ยังไม่เปิด API ที่บอกว่าออเดอร์ใดอยู่ในรอบถอนของตลาดไทย จึงยังสร้าง RC ตามรอบถอนไม่ได้ และระบบจะไม่เดาจากยอดเงินหรือวันเวลา
            </DialogDescription>
          </DialogHeader>
          {withdrawalLoading ? (
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>เปิดอ่านเพื่อดึง Statement ก่อน ส่วนส่ง SML ยังคงต้องให้พนักงานกดยืนยันทีละ Statement</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-medium">อนุญาตอ่านข้อมูลการเงิน</p>
                <p className="text-xs text-muted-foreground">ใช้กับตรวจระบบและดึง Statement ด้วยมือ</p>
              </div>
      ...
      <ConfirmDialog
        ...
        description={`ร้านนี้จะ${readEnabled ? '' : 'ไม่'}อนุญาตให้อ่าน Statement และ${smlEnabled ? '' : 'ไม่'}อนุญาตให้ส่ง RC เข้า SML\n\nการส่ง RC ยังต้องยืนยันทีละ Statement เสมอ และไม่มี Auto RC`}
