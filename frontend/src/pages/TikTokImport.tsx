...
function tiktokDestination(config?: TikTokConfig | null) {
...
export default function TikTokImport() {
  ...
  useEffect(() => {
    let alive = true
    client
      .get<TikTokConfig>('/api/settings/tiktok-config')
      .then((res) => {
    ...
    client
      .get<{ runs: ImportRunSummary[] }>('/api/import/tiktok/runs?limit=5')
      .then((res) => {
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="TikTok Excel"
        description={`อัปโหลดไฟล์จาก TikTok Seller Center แล้วสร้างเป็น${destination.documentName}สำหรับตรวจและส่งเข้า SML`}
      />
      ...
      {step === 'preview' && preview && (
        <>
          ...
          <Alert>
            ...
            <AlertDescription>
              ถ้าไฟล์ TikTok ครอบคลุมช่วงวันที่เดิม ระบบจะสร้างเฉพาะ Order ID ที่ยังไม่มีใน Nexflow และจะข้ามรายการซ้ำโดยไม่เขียนทับบิลเดิม
            </AlertDescription>
          </Alert>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-foreground">
                    กำลัง{destination.action}จาก TikTok Excel
                  </p>
